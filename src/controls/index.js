...
const shipMovementSpeed = 0.007;
let turboBoost = 0;

export function resetGame(xAxis, yAxis, zAxis, shipPosition) {
  console.log('reset')
...
export function updateSpaceshipAxis(xAxis, yAxis, zAxis, shipPosition, camera) {
  ...
  jawAngularVelocity =
    Math.abs(jawAngularVelocity) > maxAngularVelocity
      ? Math.sign(jawAngularVelocity) * maxAngularVelocity
      : jawAngularVelocity;
  pitchAngularVelocity =
    Math.abs(pitchAngularVelocity) > maxAngularVelocity
      ? Math.sign(pitchAngularVelocity) * maxAngularVelocity
      : pitchAngularVelocity;
  ...
  shipPosition.add(
    zAxis.clone().multiplyScalar(-shipMovementSpeed - turboSpeed)
  );
}
